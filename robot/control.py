...
if __name__ == "__main__":
    ...
    try:
        ...
        eki.write_variable("SYNC_VAR", 0)

        write_xml("robotControl.xml", {"SYNC_VAR": 0, "CELL_SEL":-1})

        while True:
            SYNC_VAR = int(eki.read_variable("SYNC_VAR").get('Value'))
            CELL_SEL_ASSETS = int(eki.read_variable("CELL_SEL").get('Value'))

            logging.info(f"SYNC_VAR: {SYNC_VAR}, CELL_SEL: {CELL_SEL_ASSETS}")
